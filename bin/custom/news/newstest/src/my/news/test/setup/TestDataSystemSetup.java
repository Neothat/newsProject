...
import my.news.test.constants.NewsTestConstants;
import my.news.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = NewsTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/newstest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/newstest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/newstest/import/cstickets.impex");
			...
			importImpexFile(context, "/newstest/import/csquotes.impex");
		}
	}
}
